...
TipoRet dropTable (string nombre);
TipoRet dropCol ( string nomTabla, string nombre ) ;
...
TipoRet dropTable(string nombre){
	...
	tabla T=buscartabla(nombre,inicio);
	tabla t=T;
	tabla anterior= t;
	tabla borrar = t;
	 while((T != NULL) && (borrar->nombre != nombre))
	{
		anterior = borrar;
		borrar = borrar->sig;
	}
...
TipoRet dropCol(string nomTabla,string nombre)
{
	tabla T=buscartabla(nomTabla,inicio); //primer elemento
	ptrtupla t=T->tupla;
	ptrtupla borrar=t;
	ptrtupla anterior=t;
